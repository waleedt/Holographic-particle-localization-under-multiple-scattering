function [xhat, outs] = fistaEst_E2(forwardObj, tau, Holo2D, save_every_f,results_dir_iter, varargin)
...
save_f_every_x_iters = save_every_f;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Parse input
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while(1)
    ...
    grad = forwardObj.Grad(resid_2D,s,E_rb_all,SF);
    xhatnext = proxTV(s-step*grad, step*tau, gradObj);
    %xhatnext = wthresh(xhatnext,'s',0.01);
    ...
    [data,~,~,~] = evaluateFidCost(xhatnext,Holo2D,forwardObj);
    quad = d...
    ...
    [d,z_est,SF,E_rb_all] = evaluateFidCost(s,Holo2D,forwardObj);
