%% compute scattered field uk_s from total field uk
E_rb_all_s = zeros(nx,ny,nz,length(born_orders));
for l = 2:length(born_orders)
    E_rb_all_s(:,:,:,l) = E_rb_all(:,:,:,l)-E_rb_all(:,:,:,l-1);
end

%% save uk
scatt_uk_dir = [obj_holo_dir,'\\scatt_field\\uk_scatt'];
mkdir(scatt_uk_dir);

%% save mat file of uk
uk_s_scatt_matfile = [scatt_uk_dir,'\uk_s_fields_all'];
save(uk_s_scatt_matfile,'E_rb_all_s');

%% plot and save some statistics of the norm of uk

% l2 norm of 3D field (scattered field only)
uk_s_norm = zeros(length(born_orders));
for l = 1:length(born_orders)
    uk_s = E_rb_all_s(:,:,:,l);
    uk_s_norm(l) = norm(uk_s(:), 2);
end
gca7 = figure(7);
plot(born_orders,uk_s_norm,'-*');
for l=1:length(born_orders)
    strpts = num2str(uk_s_norm(l));
    text(born_orders(l),uk_s_norm(l),strpts);
end
title('L2 norm of 3D field (scatt field only) in the object vs Born order');
xlabel('Born order');
ylabel('Performance metric');
grid on;
filename =[scatt_uk_dir,'\3D_scatt_fields_l2norm_',num2str(num_particles(i)),'.png'];
...
title('l2 norm of 2D slices of uk_s, vs slice number, for all born orders');
...
% real
for l = 1:length(born_orders)
    real_cube = real(E_rb_all_s(:,:,:,l));
    maxval = max(real_cube(:));
    minval = min(real_cube(:));
    for ll = 1:nz
        imagesc(real(E_rb_all_s(:,:,ll,l)));
        colorbar
        if (minval ~= maxval)
            caxis([minval maxval]);
        end
        title(sprintf('real scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_re_zw_dir{l},'\u',num2str(born_orders(l)),'_s_re_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end
% imag
for l = 1:length(born_orders)
    imag_cube = imag(E_rb_all_s(:,:,:,l));
    maxval = max(imag_cube(:));
    minval = min(imag_cube(:));
    for ll = 1:nz
        imagesc(imag(E_rb_all_s(:,:,ll,l)));
        colorbar
        if (minval ~= maxval)
            caxis([minval maxval]);
        end
        title(sprintf('imag scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_im_zw_dir{l},'\u',num2str(born_orders(l)),'_s_im_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end
% abs
for l = 1:length(born_orders)
    abs_cube = abs(E_rb_all_s(:,:,:,l));
    maxval = max(abs_cube(:));
    minval = min(abs_cube(:));
    for ll = 1:nz
        imagesc(abs(E_rb_all_s(:,:,ll,l)));
        colorbar
        if (minval ~= maxval)
            caxis([minval maxval]);
        end
        title(sprintf('abs scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_abs_zw_dir{l},'\u',num2str(born_orders(l)),'_s_abs_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end
...
for ll = 1:nz
    ...
    for l = 1:length(born_orders)
        ...
        title(sprintf('real scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_re_bw_dir{ll},'\u',num2str(born_orders(l)),'_s_re_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end
% imag
for ll = 1:nz
    imag_cube = imag(E_rb_all_s(:,:,ll,:));
    maxval = max(imag_cube(:));
    minval = min(imag_cube(:));
    for l = 1:length(born_orders)
        imagesc(imag(E_rb_all_s(:,:,ll,l)));
        ...
        title(sprintf('imag scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_im_bw_dir{ll},'\u',num2str(born_orders(l)),'_s_im_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end
% abs
for ll = 1:nz
    abs_cube = abs(E_rb_all_s(:,:,ll,:));
    maxval = max(abs_cube(:));
    minval = min(abs_cube(:));
    for l = 1:length(born_orders)
        imagesc(abs(E_rb_all_s(:,:,ll,l)));
        ...
        title(sprintf('abs scatt field,born:%d,slice:%d',born_orders(l),ll));
        filename =[scatt_uk_abs_bw_dir{ll},'\u',num2str(born_orders(l)),'_s_abs_slice_',num2str(ll),'.png'];
        saveas(gca8,filename,'png');
    end
end