function [Holo_est, Holo_Efield_2D, E_rb_all, Perf_all]=...
    ...
    born_orders,F_nopad,F_pad,Ft,is_rm_alias,...
...
for k = born_orders
    ...
    if (phase3D_H == 1)
        cEsp=sum(F(f.*E_rb_all(:,:,:,k)).*phase3D_G(:,:,Nzg_mid+1:end),3).*Pupil;
    else
        cEsp=sum(F(f.*E_rb_all(:,:,:,k)).*phase3D_H,3).*Pupil;
    end
    ...
    if isE2
        Holo_est(:,:,k) = abs(Avg_amp+Holo_Efield_2D).^2;
    else
        Holo_est(:,:,k) = real(Holo_Efield_2D);
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
