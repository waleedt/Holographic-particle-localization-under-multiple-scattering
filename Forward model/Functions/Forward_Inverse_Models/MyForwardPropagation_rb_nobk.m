function [Holo_est, Holo_Efield_2D, E_rb_all, Perf_all]=...
    MyForwardPropagation_rb_nobk(f,E,Nx,Ny,Nz,phase3D_H,phase3D_G,...
...
while(k < born_order)
    ...
    for j=1:Nz
        ...
        if(~isMemOpt)
            ...
            Phase3D_G_cube = phase3D_G(:,:,Nzg_mid-(j-1):Nzg_mid-(j-1)+(Nz-1));
            if j ~= Nz
                Phase3D_G_cube(:,:,j+1:end) = 0;
            end
            cEsp(:,:,j)=sum(F_Ef.*Phase3D_G_cube,3);
        end
    end
